...
private:
    ...
    std::ostream* os_ptr_;
    std::vector<std::tuple<std::string,std::string,std::function<void(const std::vector<library_info>&,json_parsing_report_generator&)>>> generators_;
    size_t counter_;
public:
    ...
    void insert_generator(const std::string& heading, 
                          std::function<void(const std::vector<library_info>&,json_parsing_report_generator&)> generator)
    {
    ...
    (*os_ptr_) << "</h2>" << std::endl;
            std::get<2>(pr)(library_info_,*this);
    (*os_ptr_) << "</div>\n";
        }
        end_report();
    }
